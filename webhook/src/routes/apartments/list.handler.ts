...
import type { ApartmentListQuery, ApartmentListResponse, Apartment } from '@safetrust/types'

export interface CreateApartmentBody {
  name?: string
  location?: string
  pricePerMonth?: number | string
  promotionPercent?: number | string | null
  rooms?: number | string
...
export const listApartments = async (
  ...
  try {
    ...
    const orderClause = sortMapping[String(sort)] || sortMapping['created_at']
    ...
    const dataResult = await db.query<Apartment>(dataQuery, finalParams)
...
export const createApartment = async (
  req: Request<{}, { apartment?: Apartment } | { error: string }, CreateApartmentBody> & { user?: { uid: string } },
  res: Response
): Promise<Response> => {
  ...
  try {
    const result = await db.query<Apartment>(insertSql, [
      ownerId,
      normalizedName,
      String(description ?? ''),
      parsedPrice,
...
export const getApartmentById = async (
  req: Request<{ id: string }, { apartment?: Apartment } | { error: string }>,
  res: Response
): Promise<Response> => {
  try {
    const { id } = req.params
    const query = `
      ...
      WHERE a.id = $1
    `
    const result = await db.query<Apartment>(query, [id])
